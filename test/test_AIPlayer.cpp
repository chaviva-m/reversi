...
TEST_F(AIPlayerTest, ChooseMoveWithNegRivalMinScore) {
  vector<Cell*> possible_moves(logic_.getPossibleMoves(board_rival_negative,
                                                              WHITE));
  Point AI_choice = a_i_player_.decideOnAMove(board_rival_negative,
...
TEST_F(AIPlayerTest, ChooseMoveWithPosRivalMinScore) {
  vector<Cell*> possible_moves(logic_.getPossibleMoves(board_rival_positive,
                                                              WHITE));
  Point AI_choice = a_i_player_.decideOnAMove(board_rival_positive,
                      possible_moves, logic_);
...
TEST_F(AIPlayerTest, ChooseMoveWithNoPossibleMovesForRival) {
  vector<Cell*> possible_moves(logic_.getPossibleMoves(board_rival_no_moves,
                                                              WHITE));
  Point AI_choice = a_i_player_.decideOnAMove(board_rival_no_moves,
                      possible_moves, logic_);
  EXPECT_EQ(AI_choice, Point(3, 0));
}
